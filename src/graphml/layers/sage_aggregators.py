...
class LstmAggregator(Aggregator):
    ...
    def forward(self, input_matrix: torch.Tensor, adjacency_coo_matrix: torch.Tensor) -> torch.Tensor:
        ...
        neighbors = input_matrix.index_select(index=edge_trg_idxs, dim=0)
        aggregated_neighbors = self.__aggregate(neighbors, edge_src_idxs)

        concat = torch.cat([input_matrix, aggregated_neighbors], dim=1)
        return torch.mm(concat, self.weights_matrix)

    def __aggregate(self, neighbors_features: torch.Tensor, edge_src_idxs: torch.Tensor) -> torch.Tensor:
        ...
        last_time_step_idxs = torch.arange(lenghts.size(0), device=outputs.device) * torch.max(lenghts).item() + (lenghts - 1)
        return outputs[last_time_step_idxs]


    def __randomly_permutate_neighbors(self, neighbors_features: torch.Tensor, edge_src_idxs: torch.Tensor) -> Tuple[torch.Tensor,torch.Tensor]:
        perm = torch.randperm(edge_src_idxs.size(0),device=edge_src_idxs.device)
