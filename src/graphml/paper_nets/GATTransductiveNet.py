...
from graphml.metrics import Accuracy, Loss, Metric


class GATTransductiveNet(torch.nn.Module):
...
""" def GAT_transductive_model(
    input_feature_dim: torch.Size,
    number_of_classes: int,
    learning_rate=.005,
    weight_decay=5e-4
) -> Tuple[GATTransductiveNet, Callable[[torch.Tensor, torch.Tensor], torch.Tensor], torch.optim.Optimizer]:
    net = GATTransductiveNet(input_feature_dim, number_of_classes)
    return net, F.nll_loss, torch.optim.Adam(net.parameters(), learning_rate, weight_decay=weight_decay) """


class GATTransductiveModel():
    def __init__(self, input_feature_dim: torch.Size, number_of_classes: int, learning_rate=.005, weight_decay=5e-4):
        self._net = GATTransductiveNet(input_feature_dim, number_of_classes)
    ...
    # pull up
    def to(self, *args, **kwargs) -> GATTransductiveModel:
        self._net.to(*args, **kwargs)
        return self

    def fit(self, epochs: int, train_data: GraphData, validation_data: GraphData, *callbacks: Optional[Callable[[GATTransductiveModel, EpochStat], None]]) -> List[EpochStat]:
    ...
    # pull up
    def _internal_fit(self, epochs: int, *callbacks: Optional[Callable[[GATTransductiveModel, EpochStat], None]]) -> List[EpochStat]:
    ...
    def _train(self) -> Tuple[Metric, Metric]:
        ...
        output = self._net(self._train_data.features_vectors,
                           self._train_data.adj_coo_matrix)
        loss = self._loss_fn(
    ...
    def _evaluate(self) -> Tuple[Metric, Metric]:
        with torch.no_grad():
            ...
            output = self._net(
                self._validation_data.features_vectors, self._validation_data.adj_coo_matrix)
            loss = self._loss_fn(
